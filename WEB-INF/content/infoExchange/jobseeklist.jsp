...
<script type="text/javascript">
	$(document).ready(function(){
		$("#select").change(function(){
			...
			window.location.href="/gdzyzmm/infoExchange/jobseeklist.action?orderway="+orderway;
		});
	});
</script>
<script type="text/javascript">
...
<body> 
	<div style="max-width:1007px;min-width:970px;margin:0 auto">
		...
		<div id="tip2" title="提示" style="display:none;">
			<p>您当前的身份是${user_role}，不能发布个人求职信息</p>
		</div>
	            	...
	            	<table class="table table-striped">
	                    ...
	                    <tbody class="">
	                    	<c:if test="${!empty page.result}">
	 						<c:forEach items="${page.result}" var="jobseek" varStatus="stat"> 
	            			<tr>
	           					<td width="70%"><a href="jobseekdetail.action?id=${jobseek.user.user_id }" class="active">求职&nbsp;${jobseek.expjob}</a></td>           			
	           					<td width="10%">${jobseek.applyername}</td>
	           					<td width="10%"><fmt:formatDate value="${jobseek.apppubdate}"></fmt:formatDate></td>           					
	            				<td width="10%">${jobseek.recount}</td>  
	            			</tr>
	            			</c:forEach>
			 				</c:if>     
	                    </tbody>  
