...
<script type="text/javascript">
	$(document).ready(function(){
		$("#select").change(function(){
			...
			window.location.href="/gdzyzmm/infoExchange/techdemalist.action?orderway="+orderway;
		});
	});
</script>
<script type="text/javascript">
...
<body> 
	<div style="max-width:1007px;min-width:970px;margin:0 auto">
		...
		<div id="tip2" title="提示" style="display:none;">
			<p>您当前的身份是${user_role}，不能发布技术需求信息</p>
		</div>
	            	...
	            	<table class="table table-striped">
	                    ...
	                    <tbody class="">
	                    	<c:if test="${!empty page.result}">
	 						<c:forEach items="${page.result}" var="techdema" varStatus="stat"> 
	            			<tr>
	           					<td width="70%"><a href="techdemadetail.action?id=${techdema.id}&demanderid=${techdema.user.user_id}" class="active">寻求：&nbsp;${techdema.tdArea}</a></td>           			
	           					<td width="10%">${techdema.user.user_name}</td>
	           					<td width="10%"><fmt:formatDate value="${techdema.tdPubDate}"></fmt:formatDate></td>
	           					<td width="10%">${techdema.recount}</td>           					
	            			</tr>
	            			</c:forEach>
			 				</c:if>     
	                    </tbody>  
