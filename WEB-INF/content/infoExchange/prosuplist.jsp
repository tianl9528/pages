...
<script type="text/javascript">
	$(document).ready(function(){
		$("#select").change(function(){
			...
			window.location.href="/gdzyzmm/infoExchange/prosuplist.action?orderway="+orderway;
		});
	});
</script>
<script type="text/javascript">
...
<body> 
	<div style="max-width:1007px;min-width:970px;margin:0 auto">
		...
		<div id="tip2" title="提示" style="display:none;">
			<p>您当前的身份是${user_role}，不能发布农产品供应信息</p>
		</div>
	            	...
	            	<table class="table table-striped">
	                    ...
	                    <tbody class="">
	                    	<c:if test="${!empty page.result}">
	 						<c:forEach items="${page.result}" var="prosup" varStatus="stat"> 
	            			<tr>
	           					<td width="70%"><a href="prosupdetail.action?id=${prosup.id}&supplyerid=${prosup.user.user_id}" class="active">供应：&nbsp;${prosup.spName}</a></td>           			
	           					<td width="10%">${prosup.user.user_name}</td>
	           					<td width="10%"><fmt:formatDate value="${prosup.spPubDate}"></fmt:formatDate></td>
	           					<td width="10%">${prosup.recount}</td>     					
	            			</tr>
	            			</c:forEach>
			 				</c:if>     
	                    </tbody>  
