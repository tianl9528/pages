...
<script type="text/javascript">
	$(document).ready(function(){
		$("#select").change(function(){
			...
			window.location.href="/gdzyzmm/infoExchange/logisupplist.action?orderway="+orderway;
		});
	});
</script>
<script type="text/javascript">
...
<body> 
	<div style="max-width:1007px;min-width:970px;margin:0 auto">
		...
		<div id="tip2" title="提示" style="display:none;">
			<p>您当前的身份是${user_role}，不能发布物流供应信息</p>
		</div>
		            	...
		            	<table class="table table-striped">
		                    ...
		                    <tbody class="">
		                    	<c:if test="${!empty page.result}">
		 						<c:forEach items="${page.result}" var="logisupp" varStatus="stat"> 
		            			<tr>
		           					<td width="70%"><a href="logisuppdetail.action?id=${logisupp.id}&supplyerid=${logisupp.user.user_id}" class="active">供应车源：&nbsp;&nbsp;${logisupp.startPlace}&nbsp;至&nbsp;${logisupp.arrivePlace}&nbsp;&nbsp;途经：&nbsp;${logisupp.wayPoint}</a></td>           			
		           					<td width="10%">${logisupp.user.user_name}</td>
		           					<td width="10%"><fmt:formatDate value="${logisupp.lsPubDate}"></fmt:formatDate></td>
		           					<td width="10%">${logisupp.recount}</td>           					
		            			</tr>
		            			</c:forEach>
				 				</c:if>     
		                    </tbody>  
