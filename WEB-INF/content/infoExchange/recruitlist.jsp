...
<script type="text/javascript">
	$(document).ready(function(){	
		$("#select").change(function(){
			...
			window.location.href="/gdzyzmm/infoExchange/recruitlist.action?orderway="+orderway;
		});
	});
</script>
<script type="text/javascript">
...
<body> 
	<div style="max-width:1007px;min-width:970px;margin:0 auto">
		...
		<div id="tip2" title="提示" style="display:none;">
			<p>您当前的身份是${user_role}，不能发布企业招聘信息</p>
		</div>
		            	...
		            	<table class="table table-striped">
		                    ...
		                    <tbody class="">
		                    	<c:if test="${!empty page.result}">
		 						<c:forEach items="${page.result}" var="recruit" varStatus="stat"> 
		            			<tr>
		           					<td width="70%"><a href="recruitdetail.action?id=${recruit.id}" class="active">招聘：&nbsp;${recruit.job}</a></td>           			
		           					<td width="10%">${recruit.user.company.companyname}</td>
		           					<td width="10%"><fmt:formatDate value="${recruit.pubdate}"></fmt:formatDate></td>
		           					<td width="10%">${recruit.recount}</td>            					
		            			</tr>
		            			</c:forEach>
				 				</c:if>     
		                    </tbody>  
