			...
			<div class="panel panel-default col-lg-12 col-xs-12"
				...
				<div class="panel-body" style="padding: 5px 15px 45px 15px">
					<table class="table table-hover " style="margin: 0">
						<c:if test="${empty page_agrimete.result}">没有数据！！！</c:if>
						<c:if test="${!empty page_agrimete.result}">
							<c:forEach items="${page_agrimete.result}" varStatus="stat"
								var="article">
								<tr>
				...
				<div class="page">
					<div class="pageBox">
						<table border="0" cellpadding="0" cellspacing="0"
							...
							<tr>
								<td><c:if test="${!empty page_agrimete.result}">${ct_agrimete}</c:if></td>
							</tr>
						</table>
					</div>
				</div>
