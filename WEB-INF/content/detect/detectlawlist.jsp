...
<body style="background: none;">
	...
	<div class="listBox" style="width: 790px">
		<div class="listTitle" style="width: 790px">
			质检法规信息录入<span class="more"><div style="float: left">
					<input type="button" value="新增" class="btn btn-success"
						style="width: 88px; margin-right: 5px"
						onClick="window.location.href='<%=request.getContextPath()%>/detect/detectlawadd.action'" />
				</div></span>
		</div>
					...
					<c:forEach items="${page_law.result}" var="law" varStatus="stat">
						<tr
							...
							<td class="trainLeft" width="120"><a
								href="<%=request.getContextPath() %>/detect/lawdetail.action?id=${law.id }"
								target="_blank" style="cursor: pointer; text-decoration: none">${law.ltitle}</a></td>
							<td class="trainLeft" width="50">${law.ltype}</td>
							<td class="trainFunction" width="80"><a
								href="<%=request.getContextPath() %>/detect/detectlawedit.action?id=${law.id}"
								class="edit">编辑</a><a
								href="<%=request.getContextPath() %>/detect/detectlawdel.action?id=${law.id}"
