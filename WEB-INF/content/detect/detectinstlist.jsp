...
<body style="background: none;">
		...
		<div class="searchBox">
			...
			<div style="float: left">
				<input type="button" value="新增" class="base_btn"
					style="float: left;"
					onClick="window.location.href='<%=request.getContextPath()%>/detect/detectinstadd.action'" />
			</div>
		</div>
		<div class="clear"></div>
	</form> -->
	...
	<div class="listBox" style="width: 790px">
		<div class="listTitle" style="width: 790px">
			鉴定机构信息录入<span class="more"><div style="float: left">
		...
		<div class="list" style="width: 790px">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0"
				...
				<c:if test="${!empty page_inst.result}">
					<c:forEach items="${page_inst.result}" var="inst" varStatus="stat">
						<tr
							...
							<td class="trainLeft" width="120"><a
								...
								target="_blank" style="cursor: pointer; text-decoration: none">${inst.dname}</a></td>
							<td class="trainLeft" width="50">${inst.dtype}</td>
							<td class="trainFunction" width="80"><a
								href="<%=request.getContextPath() %>/detect/detectinstedit.action?id=${inst.id}"
								class="edit" >编辑</a><a
								href="<%=request.getContextPath() %>/detect/detectinstdel.action?id=${inst.id}"
