...
function deleteConfirm(articleId){
		if(confirm("确定要删除吗？")){
			window.location="<%=request.getContextPath()%>/article/myArticle-delete.action?articleId="+articleId;
		}
}
</script>
</head>
...
<div class="searchBox">
       ...
       <input type="button" value="新增" class="base_btn" onClick="window.location.href='myArticle-add.action'"/>
      </div>
      <div class="clear"></div>
</form>
   
<div class="listBox">
        <div class="listTitle">我的文章</div>
        <div class="list">
          ...
          <c:forEach items="${page.result }" var="article" varStatus="stat">
            <tr class="<c:if test="${stat.count%2==1}">colorGrey</c:if><c:if test="${stat.count%2==0}">colorwhite</c:if>">
             ...
             <a href="<%=request.getContextPath()%>/artcomment/myArticle-detail.action?articleId=${article.id }" class="detail">查看</a>
             <a href="myArticle-edit.action?articleId=${article.id }" class="edit">编辑</a>
