...
function deleteConfirm(articleId){
		if(confirm("确定要删除吗？")){
			window.location="<%=request.getContextPath()%>/article/article-delete.action?articleId="+articleId;
		}
}
</script>
</head>
...
<div class="searchBox">
        ...
        <input type="button" value="新增" class="base_btn" onClick="window.location.href='article-add.action'"/>
      </div>
      <div class="clear"></div>
</form>
   
<div class="listBox">
        <div class="listTitle">文章管理<span class="more"><a href="article-add.action">新增</a></span></div>
        <div class="list">
          ...
          <c:forEach items="${page.result }" var="article" varStatus="stat">
            <tr class="<c:if test="${stat.count%2==1}">colorGrey</c:if><c:if test="${stat.count%2==0}">colorwhite</c:if>">
              <td width="260" class="trainContent">${article.articleTittle }</td>
              <td width="200" class="trainContent" >${article.createUser.user_name }</td>
              <td width="300" class="trainLeft"><fmt:formatDate value="${article.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
             <td class="trainFunction" width="170">
             <a href="<%=request.getContextPath()%>/artcomment/articleComment-list.action?articleId=${article.id}" class="detail">查看</a>
             <a href="article-edit.action?articleId=${article.id }" class="edit">编辑</a>
             <a href="#" onClick="deleteConfirm('${article.id }')" class="delete">删除</a></td>
            </tr>
          </c:forEach>
          </c:if>
          </table>
