...
function deleteConfirm(questionId){
		if(confirm("确定要删除吗？")){
			window.location="<%=request.getContextPath()%>/question/question-delete.action?questionId="+questionId;
		}
}
</script>
</head>
...
<div class="searchBox">
        ...
        <input type="button" value="新增" class="base_btn" onClick="window.location.href='question-add.action'"/>
      </div>
      <div class="clear"></div>
</form>
   
<div class="listBox">
        <div class="listTitle">问题管理<span class="more"><a href="question-add.action">新增</a></span></div>
        <div class="list">
          ...
          <c:forEach items="${page.result }" var="question" varStatus="stat">
            <tr class="<c:if test="${stat.count%2==1}">colorGrey</c:if><c:if test="${stat.count%2==0}">colorwhite</c:if>">
              <td class="trainContent">${question.questionTittle }</td>
              <td width="200" class="trainContent" >${question.createUser.user_name }</td>
              <td width="200" class="trainLeft"><fmt:formatDate value="${question.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
             <td class="trainFunction" width="170">
             <a href="reply-detail.action?questionId=${question.id}" class="detail">查看</a>
             <a href="question-edit.action?questionId=${question.id }" class="edit">编辑</a>
