									...
									<c:if test="${!empty page_pd.result}">
										<c:forEach items="${page_pd.result}" var="pd"
											...
											<tr
												...
												<td class="trainLeft" width="40"><a href="<%=base_root%>/infoExchange/pddetail.action?id=${pd.id}&buyerid=${pd.user.user_id}" target="_blank" class="active">求购&nbsp;${pd.pbName}</a></td>    
												<td class="trainLeft" width="40" style="font-size:12px;">${pd.user.user_name}</td>
												<td class="trainLeft" width="40" style="font-size:12px;"><fmt:formatDate value="${pd.pbExpDate}"></fmt:formatDate></td>
												<td class="trainFunction" width="80">
													<a href="<%=base_root%>/infoExchange/pddetail.action?id=${pd.id}&buyerid=${pd.user.user_id}" target="_blank" class="detail">查看</a>
													<a href="<%=base_root%>/infopublish/pdedit.action?id=${pd.id}" target="_blank" class="edit">编辑</a>
