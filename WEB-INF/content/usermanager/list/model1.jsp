					...
					<c:if test="${!empty page_ts.result}">
						<c:forEach items="${page_ts.result}" var="ts"
							...
							<tr
								...
								<td class="trainLeft" width="40"><a href="<%=base_root%>/infoExchange/tsdetail.action?id=${ts.id}&supplyerid=${ts.user.user_id}" target="_blank" class="active">提供&nbsp;${ts.tsName}技术</a></td> 
								<td class="trainLeft" width="40" style="font-size:12px;">${ts.user.user_name}</td>
								<td class="trainLeft" width="40" style="font-size:12px;"><fmt:formatDate value="${ts.tsPubDate}"></fmt:formatDate></td>
								<td class="trainFunction" width="80">
									<a href="<%=base_root%>/infoExchange/tsdetail.action?id=${ts.id}&supplyerid=${ts.user.user_id}" target="_blank" class="detail">查看</a>
									<a href="<%=base_root%>/infopublish/tsedit.action?id=${ts.id}" target="_blank" class="edit">编辑</a>
