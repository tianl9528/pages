									...
									<c:if test="${!empty page_rt.result}">
										<c:forEach items="${page_rt.result}" var="rt"
											...
											<tr
												...
												<td class="trainLeft" width="80"><a href="<%=base_root%>/infoExchange/rtdetail.action?id=${rt.id}" target="_blank" class="active">招聘：&nbsp;${rt.job}</a></td>
												<td class="trainLeft" width="80" style="font-size:12px;">${rt.user.company.companyname}</td>
												<td class="trainFunction" width="80">
												<!-- 	
													<a href="<%=base_root%>/infopublish/recruit-edit.action?id=${rt.id}" target="_blank" class="edit">编辑</a>
												 -->
