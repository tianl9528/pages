									...
									<c:if test="${!empty page_ls.result}">
										<c:forEach items="${page_ls.result}" var="ls"
											...
											<tr
												...
												<td class="trainLeft" width="80"><a href="<%=base_root%>/infoExchange/lsdetail.action?id=${ls.id}&supplyerid=${ls.user.user_id}" target="_blank" class="active">供应车源：&nbsp;&nbsp;${ls.startPlace}&nbsp;至&nbsp;${ls.arrivePlace}&nbsp;&nbsp;途经：&nbsp;${ls.wayPoint}</a></td> 
												<td class="trainLeft" width="80" style="font-size:12px;">${ls.user.user_name}</td>
												<td class="trainFunction" width="80">
												<!-- 
												<a href="<%=base_root%>/infopublish/logisupp-edit.action?id=${ls.id}" target="_blank" class="edit">编辑</a>
													 -->
