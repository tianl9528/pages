									...
									<c:if test="${!empty page_ps.result}">
										<c:forEach items="${page_ps.result}" var="ps"
											...
											<tr
												...
												<td class="trainLeft" width="80"><a href="<%=base_root%>/infoExchange/psdetail.action?id=${ps.id}&supplyerid=${ps.user.user_id}" target="_blank" class="active">供应：&nbsp;${ps.spName}</a></td> 
												<td class="trainLeft" width="80" style="font-size:12px;">${ps.user.user_name}</td>
												<td class="trainFunction" width="80">
												<!-- 
													<a href="<%=base_root%>/infopublish/proSup-edit.action?proSupID=${ps.id}" target="_blank" class="edit">编辑</a>
													 -->
