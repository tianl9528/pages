									...
									<c:if test="${!empty page_ld.result}">
										<c:forEach items="${page_ld.result}" var="ld"
											...
											<tr
												...
												<td class="trainLeft" width="80"><a href="<%=base_root%>/infoExchange/lddetail.action?id=${ld.id}&demanderid=${ld.user.user_id}" target="_blank" class="active">寻求车源：&nbsp;&nbsp;&nbsp;${ld.startPlace}&nbsp;至&nbsp;${ld.arrivePlace}</a></td> 
												<td class="trainLeft" width="80" style="font-size:12px;">${ld.user.user_name}</td>
												<td class="trainFunction" width="80">
												<!-- 
													<a href="<%=base_root%>/infopublish/logidema-edit.action?id=${ld.id}" target="_blank" class="edit">编辑</a>
												 -->
