...
<body onload="javascript:checkuser();">
	...
	<script type="text/javascript">
/* 	alert("${newuser.user_name}"+"${newuser.password}"); */
	</script>
	<!-- 添加注册完后登录用户需要的登录信息 -->
	  	 <input name="<%=Config.KEY_PASSWORD%>" type="hidden" tabindex="2" id="txtPassword" value="${newuser.password}">
		 <input name="<%=Config.KEY_USERNAME%>" type="hidden" tabindex="1" id="txtUserName" value="${newuser.user_name}"> 
		...
		<input type="hidden" name="<%=Config.KEY_LOGIN_TO_URL%>" value="<%=request.getContextPath()%>/${returnUrl}">
	</form>
</body>
</html>