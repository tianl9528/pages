...
try
{
	...
	if(""!=vs){ 
		...
		sql=manager.readUserByIdUnicom(condition,usertype,userIds);
		sButton="显示所有选择用户";
	}else{
		sql=manager.readAllUserUnicom(condition,usertype,roleID);
		sButton="显示已选择用户";
	}
	//得到部门所有的用户
...
function setClick(whoID) {
	document.getElementById(whoID).click();
}
function ensureConfirm(whoID) {
	 setClick(whoID);
	 parent.cb()
}
</script>
...
<form name="accessForm" action="selectUser_con_single.jsp" method="post">
<div id=searchdiv align="center" style="padding:3px;">
      <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" class="table_frame2">
        <tr class="table_row3">
          ...
          关键字：</td>
          <td width="40%">
          <input type="text" name="condition" value="<%=condition%>" class="input_oneborder" style="width:98%">
          <input style="display:none">
          <input type="hidden" name="roleID" value="<%=roleID %>"/>
          </td>
          <td width="30%" align="center"><input type="submit" name="search" class="button" value="查找">
		  <input type="hidden" name="viewSel" value="<%=sButton%>" onClick="parent.showSelect()"/>
		  </td>
        </tr>
      </table>
</div>
<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#B8CBE9">
		...
		<tr>
          <TH width="13%" class="list_title"><input type="checkbox" name="All" onClick="javascript:selectAll()" style="display: none"></TH>
...
<%	    
	if(list != null&&list.size()>0)
	{
		...
		for(int i=0;i<list.size();i++)
		{
			dat = (User)list.get(i);
	%>
        <tr  ondblclick="ensureConfirm('<%=dat.getUserID()%>')" onMouseOver="if (this.className!='trSelect') this.className='trOver'" 
                          onmouseout="if (this.className!='trSelect') this.className='trOut'" onclick="setClick('<%=dat.getUserID()%>')">
          <td><input type="radio" name="userId" id="<%=dat.getUserID()%>" nameVal="<%=dat.getUser_name()%>" value="<%=dat.getUserID()%>" onClick="checkBoxClick(this)"></td>
          <td><%=dat.getUserID()%><input type="hidden" name="userName" id="userName" value="<%=dat.getUser_name()%>"></td>
          <td><%=dat.getUser_name()%></td>
          <td><%
		  if(dat.getIs_dls()==0){
		  	out.print("联通");
		  }else if(dat.getIs_dls()==1){
		  	out.print("服务商");
		  }else if(dat.getIs_dls()==2){
		  	out.print("合作厅");
		  }else if(dat.getIs_dls()==3){
		  	out.print("会员");
		  }
		  %></td>
        </tr>
