...
try
{
	...
	if(""!=vs){
		...
		sql=manager.readPersonByIdUnicom(condition,usertype,userIds);
		sButton="显示所有选择用户";
	}else{
		sql=manager.readAllPersonUnicom(condition,usertype);
		sButton="显示已选择用户";
	}
	//得到部门所有的用户
...
<form name="accessForm" action="selectPerson_con_single.jsp" method="post">
...
<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#B8CBE9">
		...
		<tr>
          <TH width="13%" class="list_title">选择</TH>
...
<%	    
	...
	%>
        <tr onMouseOver="if (this.className!='trSelect') this.className='trOver'" 
          ...
          <td><input type="radio" name="userId" id="<%=dat.getId()%>" nameVal="<%=dat.getUsername()%>" value="<%=dat.getId()%>" onClick="checkBoxClick(this)"></td>
