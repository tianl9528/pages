...
<form name="thisForm" method="post" action="node_db.jsp" onSubmit="return checkforminput()">
                  ...
                  <table cellpadding="0" cellspacing="0" border="0" class="contentTB">
                    <tr class="greyColor">
                        ...
                        <select name="parentNodeId" style="width:50%">
                          ...
                          %>
                                    <option value="<%=node.getId()%>" <%=(dat.getParentNode()!=null&&dat.getParentNode().getId()==node.getId())?"selected":""%>><%=node.getName()%></option>
                          <%
                                    }
                                }
                          %>
                            </select>
                            <input type="hidden" name="groupid" value="<%=dat.getNodeGroup().getId()%>">
                    ...
                    <tr class="whiteColor">
                        ...
                        <td class="memberAddTd" align="left" width="">                  
                     	<input type="text" name="name" value="<%=dat.getName()==null?"":dat.getName()%>" class="itemManagerInput" valid="notnull" msg="请输入菜单名称！">
                        </td>
                    </tr>
                    <tr class="greyColor">
                        <td class="typeNameTd" width="">序号</td>
                        <td class="memberAddTd" align="left" width="">
                        <input name="train" type="text" value="<%=dat.getTrain()==null?"":dat.getTrain()%>" class="itemManagerInput" valid="notnull" msg="请输入菜单序号！">
                        </td>
                    </tr>
                    <tr class="whiteColor">
                        ...
                        %>
                                  <option value="<%=entity.getId()%>" <%=(dat.getEntity()!=null&&dat.getEntity().getId()==entity.getId())?"selected":""%>><%=entity.getName()%>（<%=entity.getValue()%>）</option>
                        <%
                                  }
                              }
                        %>
