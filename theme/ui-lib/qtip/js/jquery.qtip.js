...
PROTOTYPE.render = function(show) {
	...
	.append(
		...
		elements.content = $('<div />', {
			...
			'id': this._id + '-content',
			'aria-atomic': TRUE,
			'style': options.style.height?'height:'+(options.style.height-67)+'px;':'' //奥格
		})
	).append(
			// 创建工具栏 奥格
			elements.bbar = $('<div />', {
				'class': NAMESPACE + '-ex-bbar',
				'id': this._id + '-ex-bbar',
				'aria-atomic': TRUE
			})
	);
...
//奥格
PROTOTYPE._createButtons = function() {
	var that = this,
		buttons = this.options.buttons;
	
	elements.uiButtonSet = $("<div>")
		.addClass("buttons")
		.appendTo( elements.bbar );

	if ( $.isEmptyObject( buttons ) || ($.isArray( buttons ) && !buttons.length) ) {
		elements.bbar.removeClass(NAMESPACE + '-ex-bbar');
		return;
	}
	...
	$.each( buttons, function( name, props ) {
		...
		//	click.apply( that.elements, arguments );
		//};
		buttonOptions = {
			icons: props.icons,
			text: props.showText
